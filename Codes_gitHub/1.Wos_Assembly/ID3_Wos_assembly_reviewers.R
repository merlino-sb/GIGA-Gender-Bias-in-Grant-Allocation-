...
##-- Upload and convert WoS row data to R dataframe
 ...
 getwd()
 setwd("C:/Users/Stefano/Desktop/GIGA_Project/WoS_data_reviewers/")
...
#-- Save the clean WoS dataframe for external reviewers 
...
#*** HERE WE SAVED "WoS_data_reviewers_original.RData" ***
  
  
  
#-- List of individuals for matching   

 Reviewers_names <- read_excel("C:/Users/Stefano/Desktop/GIGA_Project/Eurocores_Kernel_Main.xlsx", sheet = "Kernel_ExternalReviewer")
  
 # Selected list of variables; remove observations with missing surname and keep only reviewers who submitted the report
 Reviewers_names$ReviewStatus <- toupper(Reviewers_names$ReviewStatus)
 Reviewers_names <- Reviewers_names %>%
   ...
   distinct(Surname, Name, ProjectYear, .keep_all = TRUE)

 # Harmonize variable format
 Reviewers_names$Surname <- toupper(Reviewers_names$Surname)
 Reviewers_names$Surname <- iconv(Reviewers_names$Surname, "UTF-8", "ASCII//TRANSLIT")
 
 Reviewers_names$Name <- toupper(Reviewers_names$Name)
 Reviewers_names$Name <- iconv(Reviewers_names$Name, "UTF-8", "ASCII//TRANSLIT")
 
 Reviewers_names$ProjectYear <- as.character(Reviewers_names$ProjectYear)
  
 # Create the variable AU (Surname + First letter) - used as key for merging
 Reviewers_names$Name_first_let <- substr(Reviewers_names$Name, 1, 1)
 Reviewers_names$AU <- paste(Reviewers_names$Surname, Reviewers_names$Name_first_let, sep = " ")
 Reviewers_names$AU <- as.character(Reviewers_names$AU)
  

  
#-- Prepare the WoS data for matching with external reviewers
...
##-- Merge WoS and reviewers dataframes  
 
 # Attach the variable 'ProjectYear' to WoS data
 project_year <- Reviewers_names %>%
 ...
 xxx <- xxx %>%
   mutate(TCN = (as.numeric(TC) / (2020 - as.numeric(PY))) * (as.numeric(ProjectYear) - as.numeric(PY)))
 ...
 # Merge by AU
  matched_data <- left_join(Reviewers_names, xxx, by = c("AU", "ProjectYear"))
  
 # Note: the data contains some NA when there are no publications before a given project application year or merging didn't give any result
...
##-- Attach the two dataframes and create the final dataset
 ...
 publication_activity_reviewers <- Reviewers_names
 ...
 publication_activity_reviewers <- left_join(Reviewers_names, matched_data_1, by = c("AU", "ProjectYear"))
 publication_activity_reviewers <- left_join(publication_activity_reviewers, matched_data_2, by = c("Surname", "ProjectYear"))
 
 publication_activity_reviewers <- publication_activity_reviewers %>%
 ...
 # Convert NAs to 0   
 publication_activity_reviewers$Publication_stock[is.na(publication_activity_reviewers$Publication_stock)] <- 0 
 publication_activity_reviewers$Citation_stock[is.na(publication_activity_reviewers$Citation_stock)] <- 0
 publication_activity_reviewers$Citat_per_paper[is.na(publication_activity_reviewers$Citat_per_paper)] <- 0
...
##-- Clean the working environment 
 rm(Reviewers_names, matched_data, matched_data_1, matched_data_2, matched_data_NA, project_year, WoS_Data, xxx, wins)
  
 
 
##-- Save the final dataset  
 save.image("C:/Users/Stefano/Desktop/GIGA_Project/Publication_activity_reviewers.RData")
