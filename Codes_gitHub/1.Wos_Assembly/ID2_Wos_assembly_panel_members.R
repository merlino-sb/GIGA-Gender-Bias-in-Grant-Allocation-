...
##-- Upload and convert WoS row data to R dataframe
 ...
 getwd()
 setwd("C:/Users/Stefano/Desktop/GIGA_Project/WoS_data_panel_members/")
...
#-- Save the clean WoS dataframe for expert panels
 ...
 #*** HERE WE SAVED "WoS_data_panel_members_original.RData" ***
  
  

#-- Upload and prepare the list of panel experts for matching with WoS data 
 
 Panel_names <- read_excel("C:/Users/Stefano/Desktop/GIGA_Project/Eurocores_Kernel_Main.xlsx", sheet = "Kernel_ReviewPanel")
 ...
 Panel_names$Surname <- toupper(Panel_names$Surname)
 Panel_names$Surname <- iconv(Panel_names$Surname, "UTF-8", "ASCII//TRANSLIT")
  
 Panel_names$Name <- toupper(Panel_names$Name)
 Panel_names$Name <- iconv(Panel_names$Name, "UTF-8", "ASCII//TRANSLIT")
 
 Panel_names$ProjectYear <- as.character(Panel_names$ProjectYear)
  
 # Create the variable AU (Surname + First letter) - used as key for merging
 Panel_names$Name_first_let <- substr(Panel_names$Name, 1, 1)
 Panel_names$AU <- paste(Panel_names$Surname, Panel_names$Name_first_let, sep = " ")
 Panel_names$AU <- as.character(Panel_names$AU)
 

  
#-- Prepare the WoS data for matching with panel experts 
...
##-- Merge WoS and expert panels dataframes  
 
 # Attach the variable 'ProjectYear' to WoS data
 project_year <- Panel_names %>%
 ...
 # Merge by AU
 matched_data <- left_join(Panel_names, xxx, by = c("AU", "ProjectYear"))
...
##-- Attach the two dataframes and create the final dataset
 ...
 publication_activity_panel_members <- Panel_names
 ...
 publication_activity_panel_members <- left_join(Panel_names, matched_data_1, by = c("AU", "ProjectYear"))
 publication_activity_panel_members <- left_join(publication_activity_panel_members, matched_data_2, by = c("Surname", "ProjectYear"))
 
 publication_activity_panel_members <- publication_activity_panel_members %>%
 ...
 # Convert NAs to 0  
 publication_activity_panel_members$Publication_stock[is.na(publication_activity_panel_members$Publication_stock)] <- 0 
 publication_activity_panel_members$Citation_stock[is.na(publication_activity_panel_members$Citation_stock)] <- 0
 publication_activity_panel_members$Citat_per_paper[is.na(publication_activity_panel_members$Citat_per_paper)] <- 0
...
##-- Clean the working environment 
 rm(Panel_names, matched_data, matched_data_1, matched_data_2, matched_data_NA, project_year, WoS_Data, xxx)
  
  
  
##-- Save the final dataset   
 save.image("C:/Users/Stefano/Desktop/GIGA_Project/Publication_activity_panel_members.RData")
